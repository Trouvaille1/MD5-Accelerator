...
typedef struct {
    ...
    XMd5_wrap_uio_map maps[ MAX_UIO_MAPS ];
} XMd5_wrap_uio_info;

/***************** Variable Definitions **************************************/
static XMd5_wrap_uio_info uio_info;
...
static int uio_info_read_name(XMd5_wrap_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XMd5_wrap_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XMd5_wrap_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XMd5_wrap_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XMd5_wrap_Initialize(XMd5_wrap *InstancePtr, const char* InstanceName) {
	XMd5_wrap_uio_info *InfoPtr = &uio_info;
