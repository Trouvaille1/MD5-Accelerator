...
//------------------------Parameter----------------------
`define TV_IN_text_input "../tv/cdatafile/c.md5_wrap.autotvin_text_input.dat"
`define TV_IN_result "../tv/cdatafile/c.md5_wrap.autotvin_result.dat"
parameter ADDR_WIDTH = 6;
parameter DATA_WIDTH = 32;
parameter text_input_DEPTH = 1;
reg [31 : 0] text_input_OPERATE_DEPTH = 0;
parameter text_input_c_bitwidth = 64;
parameter result_DEPTH = 1;
reg [31 : 0] result_OPERATE_DEPTH = 0;
parameter result_c_bitwidth = 64;
parameter text_input_data_in_addr = 16;
parameter result_data_in_addr = 28;
...
reg  BREADY_reg = 0;
reg [text_input_c_bitwidth - 1 : 0] mem_text_input [text_input_DEPTH - 1 : 0] = '{default : 'h0};
reg [DATA_WIDTH-1 : 0] image_mem_text_input [ (text_input_c_bitwidth+DATA_WIDTH-1)/DATA_WIDTH * text_input_DEPTH -1 : 0] = '{default : 'hz};
reg text_input_write_data_finish;
reg [result_c_bitwidth - 1 : 0] mem_result [result_DEPTH - 1 : 0] = '{default : 'h0};
...
//write text_input reg
reg [31 : 0] write_text_input_count = 0;
reg write_text_input_run_flag = 0;
reg write_one_text_input_data_done = 0;
//write result reg
...
assign TRAN_control_write_data_finish = 1 & text_input_write_data_finish & result_write_data_finish;
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        text_input_write_data_finish <= 0;
        write_text_input_run_flag <= 0; 
        write_text_input_count = 0;
        count_operate_depth_by_bitwidth_and_depth (text_input_c_bitwidth, text_input_DEPTH, text_input_OPERATE_DEPTH);
    end
    else begin
        ...
        if (AESL_ready_reg === 1) begin
            write_text_input_run_flag <= 1; 
            write_text_input_count = 0;
        end
        if (write_one_text_input_data_done === 1) begin
            write_text_input_count = write_text_input_count + 1;
            if (write_text_input_count == text_input_OPERATE_DEPTH) begin
                write_text_input_run_flag <= 0; 
                text_input_write_data_finish <= 1;
            end
        end
    end
end

initial begin : write_text_input
    integer write_text_input_resp;
    integer process_num ;
    integer get_ack;
    ...
    integer i;
    integer j;
    reg [31 : 0] text_input_data_tmp_reg;
    wait(reset === 1);
    @(posedge clk);
    c_bitwidth = text_input_c_bitwidth;
    process_num = 0;
    ...
    while (1) begin
        ...
        if (ongoing_process_number === process_num && process_busy === 0 ) begin
            get_ack = 1;
            if (write_text_input_run_flag === 1 && get_ack === 1) begin
                process_busy = 1;
                //write text_input data 
                for (i = 0 ; i < four_byte_num ; i = i+1) begin
                    if (text_input_c_bitwidth < 32) begin
                        text_input_data_tmp_reg = mem_text_input[write_text_input_count];
                    end
                    else begin
                        for (j=0 ; j<32 ; j = j + 1) begin
                            if (i*32 + j < text_input_c_bitwidth) begin
                                text_input_data_tmp_reg[j] = mem_text_input[write_text_input_count][i*32 + j];
                            end
                            else begin
                                text_input_data_tmp_reg[j] = 0;
                            end
                        end
                    end
                    if(image_mem_text_input[write_text_input_count * four_byte_num  + i]!==text_input_data_tmp_reg) begin
                    write (text_input_data_in_addr + write_text_input_count * ceil_align_to_pow_of_two_four_byte_num * 4 + i * 4, text_input_data_tmp_reg, write_text_input_resp);
                    image_mem_text_input[write_text_input_count * four_byte_num + i]=text_input_data_tmp_reg;
                    end
                end
                process_busy = 0;
                write_one_text_input_data_done <= 1;
                @(posedge clk);
                write_one_text_input_data_done <= 0;
            end   
            process_0_finish <= 1;
        end
        @(posedge clk);
        ...
        if (ongoing_process_number === process_num && process_busy === 0 ) begin
            ...
            if (write_result_run_flag === 1 && get_ack === 1) begin
                ...
                for (i = 0 ; i < four_byte_num ; i = i+1) begin
                    ...
                    if(image_mem_result[write_result_count * four_byte_num  + i]!==result_data_tmp_reg) begin
                    write (result_data_in_addr + write_result_count * ceil_align_to_pow_of_two_four_byte_num * 4 + i * 4, result_data_tmp_reg, write_result_resp);
                    image_mem_result[write_result_count * four_byte_num + i]=result_data_tmp_reg;
                    end
                end
                process_busy = 0;
...
// Read data from file 
initial begin : read_text_input_file_process 
  integer fp; 
  integer ret; 
  integer factor; 
  reg [151 : 0] token; 
  reg [151 : 0] token_tmp; 
  //reg [text_input_c_bitwidth - 1 : 0] token_tmp; 
  ...
  count_seperate_factor_by_bitwidth (text_input_c_bitwidth , factor);
  fp = $fopen(`TV_IN_text_input ,"r"); 
  if(fp == 0) begin                               // Failed to open file 
      $display("Failed to open file \"%s\"!", `TV_IN_text_input); 
      $finish; 
  end 
  read_token(fp, token); 
  ...
  while (token != "[[[/runtime]]]") begin 
      ...
      for(i = 0; i < text_input_DEPTH; i = i + 1) begin 
          read_token(fp, token); 
          ...
          if (factor == 4) begin
              ...
              if (i%factor == 3) begin
                  ...
                  mem_text_input [i/factor] = tmp_cache_mem;
                  tmp_cache_mem [DATA_WIDTH - 1 : 0] = 0;
              end
          end
          if (factor == 2) begin
  ...
  while (token != "[[[/runtime]]]") begin 
      ...
      for(i = 0; i < result_DEPTH; i = i + 1) begin 
          ...
          if (factor == 4) begin
              ...
              if (i%factor == 3) begin
                  tmp_cache_mem [31 : 24] = token_tmp;
                  mem_result [i/factor] = tmp_cache_mem;
                  tmp_cache_mem [DATA_WIDTH - 1 : 0] = 0;
              end
          end
          if (factor == 2) begin
